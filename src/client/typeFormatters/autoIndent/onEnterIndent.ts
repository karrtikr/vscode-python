...
export class OnEnterIndent implements OnTypeFormattingEditProvider {
    ...
    public provideOnTypeFormattingEdits(
        ...
        editor.edit((editBuilder: TextEditorEdit) => {
            newLineAndIndent(editor, editBuilder);
        });
        return [];
    }
}

export function newLineAndIndent(textEditor: TextEditor, edit: TextEditorEdit): void {
    ...
    const insertionPoint = new Position(position.line, position.character);
    const currentLine = textEditor.document.lineAt(position).text;
    let snippetCursor = '$0';
    const settings = workspace.getConfiguration('pythonIndent');
    ...
    try {
        if (textEditor.document.languageId === 'python') {
            const lines = textEditor.document.getText(new Range(0, 0, position.line, position.character)).split('\n');
        ...
        if (hanging === Hanging.Full) {
            ...
            textEditor.insertSnippet(new SnippetString(`\n${' '.repeat(tabSize)}${snippetCursor}\n`));
        } else {
            edit.insert(insertionPoint, toInsert);
        }
        textEditor.revealRange(new Range(position, new Position(position.line + 2, 0)));
    }
}
...
export function editsToMake(
...
): { insert: string; deletes: Range[]; hanging: Hanging } {
    ...
    if (numCharsToDelete > 0 && /\S/.test(currentLine.slice(0, charNum))) {
        deletes.push(new Range(lineNum, charNum, lineNum, charNum + numCharsToDelete));
    }
    ...
    if (dedentAmount > 0 || shouldTrim) {
        ...
        deletes.push(new Range(lineNum, 0, lineNum, totalDeleteAmount));
    ...
    return { insert: toInsert, deletes, hanging };
}
